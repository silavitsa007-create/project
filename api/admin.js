...
async function handleDashboard(req, res) {
  ...
  const { data: recentRaw } = await supabase
    ...
    .select('borrow_id, request_date, users!borrow_transactions_user_id_fkey(full_name), books(title)')
...
async function handleBorrowRequests(req, res, admin, body) {
  ...
  const { data, error } = await supabase
    .from('borrow_transactions')
    .select('borrow_id, request_date, requested_days, users!borrow_transactions_user_id_fkey(full_name, phone), books(book_code, title, author)')
...
async function handleReturns(req, res, admin, body) {
  ...
  const { data, error } = await supabase
    .from('borrow_transactions')
    .select('borrow_id, approve_date, due_date, users!borrow_transactions_user_id_fkey(full_name, phone), books(book_code, title)')
